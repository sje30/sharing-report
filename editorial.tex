...
of these computations are complex and in general cannot be concisely
...
documentation to allow at least key figures from your manuscript to
...
tables.  
% On the other hand, code that is not novel because it is
% already available, or that you feel that is unlikely to be of use to
% others need not be shared.  
Code that already exist, is well tested and documented, and is reused 
in the analysis should be cited. Ideally, all other code should be 
communicated, 
% This includes code that performs simple
including code that performs simple
...
analyses are performed, find potential mistakes, and reproducibility. 
...
We aslo would be part of a community and benefit from the code shared by
...
\item [Version control] Use a version control system (such as Git) to
  ...
  of publication, although we recommend opening the code from the start of the project. 
  It also makes it easy for others to contribute to
...
% should also be released.  For small datasets, this could be stored
  should also be released or made available.  For small datasets, this could be stored
...
Leveque, R. 2013. Top ten reasons to not share your code (and why you should anyway). SIAM News, April 2013, \url{http://sinews.siam.org/DetailsPage/tabid/607/ArticleID/386/Top-Ten-Reasons-To-Not-Share-Your-Code-and-why-you-should-anyway.aspx}
...
Software Carpentry \cite{wilson_best_2014} or Data Carpentry and through national neuroinformatics
...
and part of the publication in which should be shared by default, and that there should be an
