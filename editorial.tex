...
\item [Include some tests] Testing the code has long been recognized as a critical step in software industry but the practice is not widely adopted yet for small research code pieces. We recommend to include tests demonstrating that the code is indeed producing the correct results. These tests can be thought at a low level (testing each individual functions, called unit testing) or at higher level (e.g. testing that the program yields correct answers on simulated data). 
...
Stodden, V., Leish, F., \& Peng R. (Eds.). (2014). Implementing Reproducible Research. CRC press, Chapman and Hall. \cite{victoria_stodden_implementing_2014}
