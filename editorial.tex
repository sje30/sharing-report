...
\item [Include some tests] Testing the code has long been recognized as a critical step in software industry but the practice is not widely adopted yet for research code. We recommend to include tests demonstrating that the code is indeed producing the correct results. These tests can be thought at a low level (testing each individual function, called unit testing) or at higher level (e.g. testing that the program yields correct answers on simulated data) \cite{wilson_best_2014}. 
